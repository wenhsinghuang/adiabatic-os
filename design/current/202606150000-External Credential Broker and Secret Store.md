...
`managedProvider` is the official Lamarck-managed path for confidential apps and provider-specific OAuth. The hosted service owns the provider client id, client secret, provider token endpoint, redirect URI, provider review/compliance, refresh-token custody, refresh behavior, browser OAuth ceremony, and provider API proxy. The local manifest does not carry provider OAuth metadata or hosted URLs. The core resolves `providerId` through Lamarck system configuration: `app.lamarck.ai/providers/{providerId}/connect` owns login, account, and provider connect UI; `api.lamarck.ai/providers/{providerId}/v1/*` owns connector-facing provider data APIs. Provider APIs should be Lamarck-defined allowlisted surfaces, not raw upstream API mirrors. For example, the Oura reference provider exposes `GET /providers/oura/v1/streams/{streamId}` and maps those stable Lamarck stream IDs to Oura V2 `/v2/usercollection/*` endpoints server-side.
...
The local credential broker / secret store still brokers runtime access. For `managedProvider`, `ConnectorAuthManager` stores only the managed credential binding: `{ kind: "managedProvider", providerId, integrationId }`. It does not store provider tokens and does not persist a Lamarck capability-token cache in the first implementation. The hosted service remains the durable owner of provider refresh tokens and performs provider refresh server-side. Each `getToken()` call uses the local Lamarck desktop session inside the credential module to request a fresh 24-hour opaque Lamarck capability token. Connector code receives only that scoped capability token, never the desktop session token and never the provider OAuth token.

Managed-provider server state is keyed by `userId + integrationId`. `integrationId` is the workspace-logical connector account/source identity; multiple provider accounts are represented as multiple connector integrations. `workspaceId` is intentionally not required by this contract yet. When workspace sync introduces a durable workspace identity and ACL model, it can be added as optional grouping/authorization metadata without changing the connector runtime contract.
...
For direct `oauth2-public`, the OAuth2 secret blob holds the provider access token, refresh token, and expiry together; `auth_credentials.expires_at` duplicates the expiry in clear so the scheduler can decide to refresh without decrypting. For `managedProvider`, the blob holds only the managed binding (`providerId + integrationId`), not the provider refresh token and not a durable capability token. `getToken()` is the capability-token issue point. Concurrent `getToken()` calls may single-flight in memory for the same credential, but the capability token is not persisted as local credential state.
...
- **Managed providers for official connector OAuth** — need provider-specific hosted OAuth, provider token vaulting, and provider API proxy implementations. The shared local/server capability-token protocol is part of the `managedProvider` contract: local stores `providerId + integrationId`; server stores provider token state under `userId + integrationId`; connector runtime receives only scoped Lamarck capability tokens for `/providers/{providerId}/v1/*`.
