...
Status: canon. This doc owns the **schema design** for D0 (`events`), D1 (`docs`), and D2; the literal table definitions live in `desktop/core/src/db.ts`. **Storage layout — which table lives in which database — is owned by [System Database Separation](202606150100-System%20Database%20Separation.md), not here:** `events`/`docs` are the substrate and live in the data DB; `connector_integrations` / `connector_custom_approvals` are system runtime state (not substrate) and live in the system DB (see Connector Runtime Module). D0 payloads have since evolved (two-way door): `d1.write` stores a unified-diff `patch` (the "以後加 diff" option below, since adopted), and `ddl.promote`/`ddl.demote` store full before/after schema snapshots — see `guard.ts`.
...
| `schema_version` | TEXT NOT NULL | D0 event schema 的版本。第一版也必填，因為 raw event 是最硬的 one-way door；未來欄位改名、語義修正、parser 分流、lazy migration 都靠它保留改動權。 |
...
**Raw 永遠贏。** D0 只記 raw event，不把 aggregate、trait、summary 這類 derived 結果混進 raw schema。D2 可以存 derived state，但 derived row 必須能說清楚：

- `derived_from_event_ids` 或等價 provenance
- `derived_by_version`（derive code / prompt / model / rule 的版本）
- 可丟棄、可重算，不是 source of truth

這是 substrate 保留未來改動權的核心：derive 邏輯可以重寫，D2 schema 可以重建，但 D0 event log 五年後仍能被讀出來並重新投影。

### Schema Evolution

D0 schema 變更採用「版本標籤 + lazy migration」作為 default pattern：

- 新 event 用新的 `schema_version`
- 舊 event 不改寫，讀取時由 parser 按 version normalize
- 小變更（欄位改名、default 值）在讀取層 transform
- 大變更可以寫 migration event，讓 schema boundary 本身也成為 D0 fact

禁止沒有 version 的 raw event 進 D0。沒有 version 就沒有安全的 parser 分流，也就沒有便宜的 substrate evolution。
...
- version 欄位只放在 D0，因為 D0 是永久 event log；D1 是工作面，metadata JSON 就夠
