...
Status: canon (verified against `core/src/db.ts` 2026-06-10). The `events` and `docs` tables match exactly; implementation adds append-only enforcement triggers on `events`. Known two-way-door drift in D0 payload details: `d1.write` now stores a unified-diff `patch` (the "以後加 diff" option below, since adopted), and `ddl.promote`/`ddl.demote` store full before/after schema snapshots — see `guard.ts`. `connector_integrations` / `connector_custom_approvals` tables also live in the same DB but are system runtime state, not substrate (see Connector Runtime Module).
